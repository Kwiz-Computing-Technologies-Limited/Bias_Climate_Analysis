thin_modified <- function( loc.data, lat.col="LAT", long.col="LONG", spec.col="SPEC",
                  thin.par, reps,
  ...
  source(here("bias_correction", "thin.algorithm_modified.R"))
  
  # Keep track of how much time it takes to run this algorithm
  gc()
  thin.time <- system.time( 
    locs.thinned <- thin.algorithm_modified( rec.df.orig=locs.long.lat,
  ...
  if( write.files ){
    ...
    csv.files <- paste( out.dir, out.base, gsub(" ", "_", as.character(species)), "_thin_", rep(1:n.csv), 
                        ".csv", sep="")
