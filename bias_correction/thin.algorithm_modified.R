...
thin.algorithm_modified <- function(rec.df.orig, thin.par, reps) {
  ...
  ## ***
  ## Distances calculated using fields::RdistEarth (an alternative to fields::RdistEarth()). This function
  ## which returns distances, correcting for change in distance
  ## of degree of longitude based on distance from equator
  ## is written in C and is memory efficient compared to fields::RdistEarth()
  
  DistMat.save <- RdistEarth(x1=as.matrix(rec.df.orig), miles=FALSE) < thin.par
