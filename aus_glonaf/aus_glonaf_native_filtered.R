...
drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AAAF1mnPJIdN1ogrQRQqr0fZa/AUS-Glonaf/AUS-Glonaf-native-filtered-v1.csv?dl=1"
country = "aus_glonaf"
...
for (n in N) {
  ...
  tryCatch({
    if(!("aus_glonaf_native_filtered" %in% dbListTables(aws_con))){
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "aus_glonaf_native_filtered", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM aus_glonaf_native_filtered")$count
