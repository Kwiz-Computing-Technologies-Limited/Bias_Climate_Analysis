...
# Get nzl_glonaf native clean


drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AACg4EsovSUZOHwpc-bWaG0Da/NZL-Glonaf/NZL-Glonaf-native-clean.csv?dl=1"
country = "nzl_glonaf"
habitat = "native-clean"
...
for (n in N) {
  ...
  tryCatch({
    if(!("nzl_glonaf_native_clean" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "nzl_glonaf_native_clean", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM nzl_glonaf_native_clean")$count
