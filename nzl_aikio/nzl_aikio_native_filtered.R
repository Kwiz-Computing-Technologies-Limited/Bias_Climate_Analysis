...
# Get nzl_aiko native filtered


drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AAB8JaYFJg9mW1CSEc7_36Zza/NZL-Aikio/NZL-Aikio-native-filtered.csv?dl=1"
country = "nzl_aiko"
...
for (n in N) {
  ...
  tryCatch({
    if(!("nzl_aiko_native_filtered" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "nzl_aiko_native_filtered", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM nzl_aiko_native_filtered")$count
