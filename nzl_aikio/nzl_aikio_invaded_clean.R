...
# Get nzl_aiko invaded clean


drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AABImLpXuN4ANJ5SIIoNvvJla/NZL-Aikio/NZL-Aikio-invaded-clean.csv?dl=1"
country = "nzl_aiko"
...
for (n in N) {
  ...
  tryCatch({
    if(!("nzl_aiko_invaded_clean" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "nzl_aiko_invaded_clean", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM nzl_aiko_invaded_clean")$count
    ...
    if(((x/1000000) - floor(x/1000000)) == 0) {
      ...
      if(!is_empty(value)) {
        ...
        db_columns = dbGetQuery(conn=aws_con, statement = "SELECT column_name 
                        FROM information_schema.columns WHERE table_name = 'nzl_aiko_invaded_clean' 
