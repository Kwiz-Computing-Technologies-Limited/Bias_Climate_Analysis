...
drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AAAhkJh8YKNvFD7orziL4XZya/NZL-Aikio/NZL-Aikio-native-clean.csv?dl=1"
country = "nzl_aiko"
...
for (n in N) {
  ...
  tryCatch({
    if(!("nzl_aiko_native_clean" %in% dbListTables(aws_con))){
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "nzl_aiko_native_clean", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM nzl_aiko_native_clean")$count
