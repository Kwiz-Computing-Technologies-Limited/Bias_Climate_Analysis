...
# Get mdg native clean


drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AABqyaNLd2vTj-nwBJpBBmA2a/MDG/MDG-native-clean.csv?dl=1"
country = "mdg"
...
for (n in N) {
  ...
  tryCatch({
    if(!("mdg_native_clean" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "mdg_native_clean", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM mdg_native_clean")$count
