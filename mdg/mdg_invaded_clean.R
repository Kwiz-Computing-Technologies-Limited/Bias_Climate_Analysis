...
drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AADNNQGMWaNwIJufSYG_jJABa/MDG/MDG-invaded-clean.csv?dl=1"
country = "mdg"
...
for (n in N) {
  ...
  tryCatch({
    if(!("mdg_invaded_clean" %in% dbListTables(aws_con))){
      print("fetching new")
    ...
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM mdg_invaded_clean")$count
