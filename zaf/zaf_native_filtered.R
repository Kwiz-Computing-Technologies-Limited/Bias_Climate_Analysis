...
# Get zaf native filtered


drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AABkBNRP_wnKQoTf2eMdUn9pa/ZAF/ZAF-native-filtered.csv?dl=1"
country = "zaf"
...
for (n in N) {
  ...
  tryCatch({
    if(!("zaf_native_filtered" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "zaf_native_filtered", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM zaf_native_filtered")$count
