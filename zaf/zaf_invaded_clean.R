...
# Get zaf invaded clean


drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AAByRGPsPkve-tNACI2QsNpEa/ZAF/ZAF-invaded-clean.csv?dl=1"
country = "zaf"
...
for (n in N) {
  ...
  tryCatch({
    if(!("zaf_invaded_clean" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "zaf_invaded_clean", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM zaf_invaded_clean")$count
    ...
    if(((x/1000000) - floor(x/1000000)) == 0) {
      ...
      if(!is_empty(value)) {
        ...
        db_columns = dbGetQuery(conn=aws_con, statement = "SELECT column_name 
                        FROM information_schema.columns WHERE table_name = 'zaf_invaded_clean' 
