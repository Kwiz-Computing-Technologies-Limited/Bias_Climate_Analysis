...
drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AAA_Hky88uStwNHsHu8-Vtx4a/ZAF/ZAF-native-clean.csv?dl=1"
country = "zaf"
...
for (n in N) {
  ...
  tryCatch({
    if(!("zaf_native_clean" %in% dbListTables(aws_con))){
      print("fetching new")
    ...
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM zaf_native_clean")$count
