...
# Get aus_randal invaded clean

drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AACG5bydJd8HCrcF1U2XgTLja/AUS-Randall/AUS-Randall-invaded-clean.csv?dl=1"
country = "aus_randal"
...
for (n in N) {
  ...
  tryCatch({
    if(!("aus_randal_invaded_clean" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "aus_randal_invaded_clean", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM aus_randal_invaded_clean")$count
    ...
    if(((x/1000000) - floor(x/1000000)) == 0) {
      ...
      if(!is_empty(value)) {
        ...
        db_columns = dbGetQuery(conn=aws_con, statement = "SELECT column_name 
                        FROM information_schema.columns WHERE table_name = 'aus_randal_invaded_clean' 
