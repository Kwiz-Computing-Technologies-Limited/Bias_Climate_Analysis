...
# Get gbr native clean


drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AACds29mdx_awJM1S7_tKpXfa/GBR/GBR-native-clean.csv?dl=1"
country = "gbr"
...
for (n in N) {
  ...
  tryCatch({
    if(!("gbr_native_clean" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "gbr_native_clean", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM gbr_native_clean")$count
