...
# read gbr_invaded_clean files from google drive folder
library(ggplot2)
```
...
```{r, echo =FALSE, cache=TRUE, results='asis'}
 directory_files_gbr = list.files("~/Desktop/Documents/GitHub/bias assessment/13.  bias assessment results") 
 directory_files_gbr = directory_files_gbr[(directory_files_gbr |>
   grepl(pattern = "gbr_invaded_clean"))]
```


```{r, echo =FALSE, cache=TRUE, results='asis'}
## 1. periods
file_link_gbr = paste0("~/Desktop/Documents/GitHub/bias assessment/13.  bias assessment results/", directory_files_gbr[grepl("periods_output", directory_files_gbr)])
...
for (i in 1:ncol(periods_gbr)) {
  periods_range_gbr$range[i] = paste(min(periods_gbr[, i], na.rm = T), 
                                  max(periods_gbr[ , i], na.rm = T), sep = "-")
}

periods_range_gbr$range = as.factor(periods_range_gbr$range)
```


Occurrence records for the *gbr_invaded_clean* landscape were divided into `r length(periods_gbr)` 10-year periods (`r length(periods_gbr) * 10` years) between `r min(periods_gbr[[1]])` - `r max(periods_gbr[[length(periods_gbr)]])`. These periods provides information on *temporal variations* in the different biases. We use ANOVA and the Tukey multiple comparison tests to test for these variations. The data are also grouped by their *family (taxonomic rank)* which forms the basis of the comparisons
...
file_link_gbr = paste0("~/Desktop/Documents/GitHub/bias assessment/13.  bias assessment results/", directory_files_gbr[grepl("assessSpeciesNumber_output", directory_files_gbr)])
...
Figure 1: Taxonomic coverage (Number of species) for gbr_invaded_clean occurrence data
```{r, echo =FALSE, cache=TRUE, results='asis', fig.show='asis'}
library(ggplot2)
assessSpeciesNumber_gbr |>
...
- There exists variations in the number of species identified between the different families (taxonomic groups) (Anova p-value = `r p.value_gbr` < 0.005). Therefore, there are families that received more species coverage than others.

- Out of the `r assessSpeciesNumber_gbr$Taxonomic_Group |> unique() |> length() |> choose(2)` pairs of families compared, differences were significant in `r species_differences_gbr |> nrow()` pairs. Taxonomic coverage was therefore present in `r (100 * (species_differences_gbr |> nrow()) / (assessSpeciesNumber_gbr$Taxonomic_Group |> unique() |> length() |> choose(2))) |> round(0)`% of the families compared.

- Table 1 gives the results of the multiple comparison test (tukey test) which ranks the differences in the levels of coverage between the `r species_differences_gbr |> nrow()` pairs of families. Families are ranked from ones the highest absolute difference to those with the least absolute difference.

Table 1: Differences in the number of species identified between families
```{r, echo =FALSE, cache=TRUE, results='asis'}
species_differences_gbr |> knitr::kable()
```
...
taxonomic_coverage_slope_gbr = split.data.frame(assessSpeciesNumber_gbr, assessSpeciesNumber_gbr$Taxonomic_Group)  |>
  purrr::map(~lm(val ~ 1 + Period, data = .x))|>
  purrr::map_df(tidy) |>
  dplyr::filter(term == 'Period') |>
  dplyr::mutate(term = sort(assessSpeciesNumber_gbr$Taxonomic_Group |> unique())) |>
  dplyr::filter(p.value < 0.05 | p.value == 0.05) |>
  dplyr::rename(Taxonomic_group = term) |> dplyr::arrange(desc(estimate))
```

- `r taxonomic_coverage_slope_gbr$Taxonomic_group |> unique() |> length()` out of the `r assessSpeciesNumber_gbr$Taxonomic_Group |> unique() |> length()` families (`r (100 * (taxonomic_coverage_slope_gbr$Taxonomic_group |> unique() |> length()) / (assessSpeciesNumber_gbr$Taxonomic_Group |> unique() |> length())) |> round(0)`%) had a significant temporal bias in taxonomic coverage 

Table 2: Slope estimates of the number of species identified by taxonomic group
```{r, echo =FALSE, cache=TRUE, results='asis'}
taxonomic_coverage_slope_gbr |> knitr::kable()
```
...
```{r, echo =FALSE, cache=TRUE, results='asis'}
## 4. assessRarityBias
file_link_gbr = paste0("~/Desktop/Documents/GitHub/bias assessment/13.  bias assessment results/", directory_files_gbr[grepl("assessRarityBias_output", directory_files_gbr)])
...
Figure 3(a): Taxonomic bias index (Rare species representation) for gbr_invaded_clean occurrence data
```{r, echo =FALSE, cache=TRUE, results='asis', fig.show='asis'}
assessRarityBias_gbr |>
...
rare_coverage_ranges_gbr |> knitr::kable()
```
...
-  The number of periods in which species were recorded varied from `r rare_coverage_ranges_gbr$n_periods |> min()` to `r rare_coverage_ranges_gbr$n_periods |> max()` for different families

- taxonomic coverage indices for a family varied between periods. The range between the period with the highest coverage and the period with the lowest coverage varied between `r rare_coverage_ranges_gbr$range |> min()` and `r rare_coverage_ranges_gbr$range |> max()` for different species

- different families also had different levels of taxonomic coverage. The minimum coverage index for a single 10-year period ranged between `r rare_coverage_ranges_gbr$min |> min()` and `r rare_coverage_ranges_gbr$min |> max()` for different families (a difference of `r (rare_coverage_ranges_gbr$min |> max() - (rare_coverage_ranges_gbr$min |> min()))`). Similarly, the maximum coverage index for a single 10-year period ranged between `r rare_coverage_ranges_gbr$max |> min()` and `r rare_coverage_ranges_gbr$max |> max()` for different families (a difference of `r (rare_coverage_ranges_gbr$max |> max() - (rare_coverage_ranges_gbr$max |> min()))`)
...
Table 3(b): Identifying Oversampled vs Unsersampled families in gbr_invaded_clean dataset
```{r, echo =FALSE, cache=TRUE, results='asis'}
family_residual_gbr |> knitr::kable()
```
...
for (i in 1:length(cells$year)) {
  for (j in 1:length(periods_gbr)) {
    if(cells$year[i] %in% periods_gbr[[j]]) {
      cells$period[i] = j
    }
  }
}
...
cells = cells |>
  ...
  dplyr::filter(family %in% family_residual_gbr$family) |>
  dplyr::arrange(family, period)

# count oversampled/undersampled cells 
oversampled = family_residual_gbr |>
  aggregate(oversampled ~ family + period, FUN = sum)
undersampled = family_residual_gbr |>
...
- Less than `r undersampled_rate`% and `r oversampled_rate`% of cells in gbr_invaded_clean were under-sampled and over-sampled during any time period respectively. Table 3(c) below summarises each family's range (occupied cells) and the level of over-sampling or under-sampling for different periods.

Table 3(c): Summary table for occupied, over-sampled and under-sampled cells for families in gbr_invaded_clean dataset
```{r, echo =FALSE, cache=TRUE, results='asis'}
cells |> knitr::kable()
```
...
rare_coverage_slope_gbr |> knitr::kable()
```

- We observe from table 4, that coverage of rare species did not change except in `r nrow(rare_coverage_slope_gbr)` taxonomic groups in the gbr_invaded_clean dataset where over-representation or under-representation of some species reduced over time. 
...
Figure 4: Nearest neighbour index for gbr_invaded_clean occurrence data
```{r, echo =FALSE, cache=TRUE, results='asis', fig.show='asis'}
assessSpatialBias_gbr |>
...
spatial_index_slope_gbr |> knitr::kable()
```

We also note more clustered sampling among `r nrow(spatial_index_slope_gbr)` taxonomic groups over time suggesting an increase in spatial bias within these groups.
...
Figure 5: Spatial map for gbr_invaded_clean occurrence data
```{r, echo =FALSE, cache=TRUE, results='asis', fig.show='asis'}
library(magick)
## 6. assessSpatialCov
file_link_gbr = paste0("~/Desktop/Documents/GitHub/bias assessment/13.  bias assessment results/", directory_files_gbr[grepl("assessSpatialCov_output", directory_files_gbr)])
knitr::include_graphics(file_link_gbr)
```
...
file_link_gbr = paste0("~/Desktop/Documents/GitHub/bias assessment/13.  bias assessment results/", directory_files_gbr[grepl("assessRecordNumber_output", directory_files_gbr)])
...
sampling_intensity_slope_gbr = split.data.frame(assessRecordNumber_gbr, assessRecordNumber_gbr$Taxonomic_Group) |>
  ...
  purrr::map_df(tidy) |>
  dplyr::filter(term == 'Period') |>
  dplyr::mutate(term = sort(assessRecordNumber_gbr$Taxonomic_Group |> unique())) |>
  dplyr::filter(p.value < 0.05 | p.value == 0.05) |>
  dplyr::rename(Taxonomic_group = term) |> dplyr::arrange(desc(estimate))
```

We observe significant positive slopes indicating an increase in sampling intensity among `r nrow(sampling_intensity_slope_gbr)` out of the `r length(unique(assessRecordNumber_gbr$Taxonomic_Group))` (`r (100 * nrow(sampling_intensity_slope_gbr) / length(unique(assessRecordNumber_gbr$Taxonomic_Group))) |> round(4)`) taxonomic groups in the gbr_invaded_clean occurrence dataset, as shown in table 6 below.
...
```{r, echo =FALSE, cache=TRUE, results='asis'}
sampling_intensity_slope_gbr |> knitr::kable()
```
...
records_differences_gbr = tukey_species_gbr$Taxonomic_Group[!(tukey_species_gbr$Taxonomic_Group[, "p adj"] > 0.05) , ] |> as.data.frame() |>  dplyr::arrange(desc(diff))

records_differences_gbr |> knitr::kable()
```

## correlation between sampling intensity and number of species

```{r, echo=FALSE, cache=TRUE}
correlation_gbr = cor.test(assessSpeciesNumber_gbr$val, assessRecordNumber_gbr$val)
```

We observe a positive correlation $cor =$ `r correlation_gbr$estimate` (p.value_gbr = `r correlation_gbr$p.value`). This indicates presence of sampling bias (higher sampling intensities) towards families with more species.
