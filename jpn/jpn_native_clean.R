...
drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AAA8yHeMlL-xV0EseYNrpi3Ma/JPN/JPN-native-clean.csv?dl=1"
country = "jpn"
...
for (n in N) {
  ...
  tryCatch({
    if(!("jpn_native_clean" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "jpn_native_clean", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM jpn_native_clean")$count
