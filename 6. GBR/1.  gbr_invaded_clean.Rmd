...
for (n in N) {
  ...
  if(!("gbr_invaded_clean" %in% dbListTables(aws_con))){
    ...
    paste("reading", 1000000, "rows from", dbGetQuery(aws_con, "SELECT COUNT(*) FROM gbr_invaded_clean")$count) |> print()
  start = Sys.time()
  ...
  value = read.csv(file = drop_url, header = F,
                    colClasses = c("NULL", "character","NULL", "NULL", "NULL", "NULL", 
                                   "numeric", "numeric", "NULL", "NULL", "numeric", "NULL",
                                   "numeric", "numeric", "NULL", "NULL", "NULL", "NULL", "NULL", "NULL", "NULL"), 
                   nrows = 1000000, skip = dbGetQuery(aws_con, "SELECT COUNT(*) FROM gbr_invaded_clean")$count)
     ...
     if(!is_empty(value)) {
      ...
      paste("Updating", dbGetQuery(aws_con, "SELECT COUNT(*) FROM gbr_invaded_clean")$count, "to", (dbGetQuery(aws_con, "SELECT COUNT(*) FROM gbr_invaded_clean")$count + nrow(value)), "to AWS DB") |> print()
