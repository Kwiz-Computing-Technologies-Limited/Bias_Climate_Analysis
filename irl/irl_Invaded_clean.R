...
drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AADeejaZjhcCw5wRTDATynqAa/IRL/IRL-invaded-clean.csv?dl=1"
country = "irl"
...
for (n in N) {
  ...
  tryCatch({
    if(!("irl_invaded_clean" %in% dbListTables(aws_con))){
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "irl_invaded_clean", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM irl_invaded_clean")$count
    ...
    if(((x/1000000) - floor(x/1000000)) == 0) {
      ...
      if(!is_empty(value)) {
        ...
        db_columns = dbGetQuery(conn=aws_con, statement = "SELECT column_name 
                        FROM information_schema.columns WHERE table_name = 'irl_invaded_clean' 
