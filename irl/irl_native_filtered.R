...
# Get irl native filtered


drop_url = "https://www.dropbox.com/sh/g7hbgbfyzocizmx/AAC6HWdsbJARiq8MEx7iE9ona/IRL/IRL-native-filtered.csv?dl=1"
country = "irl"
...
for (n in N) {
  ...
  tryCatch({
    if(!("irl_native_filtered" %in% dbListTables(aws_con))){
      print("fetching new")
      ...
      print("uploading new")
      dbWriteTable(conn = aws_con, name = "irl_native_filtered", value = value)
    } 
    
    x = dbGetQuery(aws_con, "SELECT COUNT(*) FROM irl_native_filtered")$count
